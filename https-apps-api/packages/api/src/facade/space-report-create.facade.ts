import { ArrayUtils, config, queue, spaceReport, user as userDomain } from '@pfda/https-apps-shared'
import type { UserOpsCtx } from '@pfda/https-apps-shared/src/types'
...
export class SpaceReportCreateFacade {
  ...
  async createSpaceReport(spaceId: number) {
    ...
    if (ArrayUtils.isEmpty(report.reportParts.getItems())) {
      return
    }
