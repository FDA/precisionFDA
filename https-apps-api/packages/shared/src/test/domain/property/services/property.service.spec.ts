...
import { PropertyService } from '../../../../domain/property'
import { PropertyType } from "../../../../domain/property/property.entity";
...
describe('app service tests', () => {
    ...
    it('save properties - target from space and user does have sufficient permissions', async () => {
        ...
        await em.flush()
        const spaceFile = create.filesHelper.create(em, {user}, {scope: "space-" + space.id})
        create.spacesHelper.addMember(em, {user: notOwner, space}, {role: SPACE_MEMBERSHIP_ROLE.CONTRIBUTOR})
        await em.flush()
    ...
    it('save properties - public target and user does have sufficient permissions', async () => {
        const publicFile = create.filesHelper.create(em, {user}, {scope: 'public'})
        const notOwnerButAdmin = create.userHelper.createSiteAdmin(em)
        await em.flush()

        const propertyService = new PropertyService(em, {...notOwnerButAdmin, accessToken: 'secretToken'})
        const input = {
    ...
    it('fail save properties - target from space and user does not have sufficient permissions', async () => {
        ...
        const spaceFile = create.filesHelper.create(em, {user}, {scope: "space-" + space.id})
        create.spacesHelper.addMember(em, {user: notOwner, space}, {role: SPACE_MEMBERSHIP_ROLE.VIEWER})
        await em.flush()
    ...
    it('get valid keys - own private target', async () => {
        ...
        const file4 = create.filesHelper.create(em, {user}, {scope: 'public'})
        ...
        const propertyServiceAnotherUser = new PropertyService(em, {...anotherUser, accessToken: 'secretToken'})
        ...
        const res = await propertyService.getValidKeys({targetType: 'node', scope: 'private'})
