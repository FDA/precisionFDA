...
describe.only('syncFilesInFolder operation', () => {
  ...
  it('creates one new file in a folder', async () => {
    ...
    expect(res[0]).to.have.property('name', createdFileDesc.name)
    expect(res[0]).to.have.property('dxid', createdFileDesc.id)
    expect(fakes.client.filesListFake.calledOnce).to.be.true()
  ...
  it('creates one more new file in a subfolder', async () => {
    ...
    expect(res[0]).to.have.property('name', file.name)
    expect(res[0]).to.have.property('dxid', firstFileDxid)
    expect(res[1]).to.have.property('name', createdFileDesc.name)
    expect(res[1]).to.have.property('dxid', createdFileDesc.id)
    expect(fakes.client.filesListFake.calledOnce).to.be.true()
    expect(fakes.client.filesDescFake.calledOnce).to.be.true()
  })
  ...
  it('deletes existing file and creates another', async () => {
    ...
    const res = await op.execute(input)
    expect(res).to.be.an('array').with.lengthOf(1)
    expect(res[0]).to.have.property('dxid', secondFileDxid)
    const nodesCount = await em.count(UserFile, {}, { filters: ['userfile'] })
    expect(nodesCount).to.be.equal(1)
  })
