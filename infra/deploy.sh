...
aws autoscaling create-auto-scaling-group \
    ...
    --tags "Key=Environment,Value=${ENVIRONMENT}" "Key=Role,Value=${ENV_INSTANCE_TYPE}" "Key=Namespace,Value=pfda" "Key=Name,Value=pfda-${ENVIRONMENT}-${ENV_INSTANCE_TYPE}-vm" "Key=CrowdStrikeExempt,Value=ASG" \
