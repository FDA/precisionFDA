...
import {
  homeFilesIsFetchingSelector,
  homeFilesIsCheckedAllSelector,
  homeFilesFiltersSelector,
  homePathSelector,
...
const breadcrumbs = (path) => (
  <div className="space-files-table__breadcrumbs">
    ...
    {
      ([{ id: 0, name: 'Files', href: '/home/files' }]
        .concat((path || [])
          .map(folder => ({
            id: folder.id,
            name: folder.name,
            href: `/home/files?folderId=${folder.id}`,
...
const HomeFilesTable = ({ files, isFetching, isCheckedAll, toggleAllFilesCheckboxes, toggleFileCheckbox, filters, handleFilterValue, path }) => {
...
const mapStateToProps = (state) => ({
  isFetching: homeFilesIsFetchingSelector(state),
  isCheckedAll: homeFilesIsCheckedAllSelector(state),
  filters: homeFilesFiltersSelector(state),
  path: homePathSelector(state),
})

const mapDispatchToProps = (dispatch) => ({
  toggleAllFilesCheckboxes: () => dispatch(toggleAllFilesCheckboxes()),
  toggleFileCheckbox: (id) => dispatch(toggleFileCheckbox(id)),
})

export default connect(mapStateToProps, mapDispatchToProps)(HomeFilesTable)
