...
import {
  homeFilesSpacesIsFetchingSelector,
  homeFilesSpacesIsCheckedAllSelector,
  homeFilesSpacesFiltersSelector,
  homePathSpacesSelector,
...
const breadcrumbs = (path) => (
  <div className="space-files-table__breadcrumbs">
    ...
    {
      ([{ id: 0, name: 'Files', href: '/home/files/spaces' }]
        .concat((path || [])
          .map(folder => ({
            id: folder.id,
            name: folder.name,
            href: `/home/files/spaces?folderId=${folder.id}`,
...
const HomeFilesSpacesTable = ({ files, isFetching, isCheckedAll, toggleAllFilesCheckboxes, toggleFileCheckbox, filters, handleFilterValue, path }) => {
...
const FolderLink = ({ file }) => {
  return (
    <Link to={{ pathname: '/home/files/spaces', search: `?folderId=${file.id}` } || ''}>
...
const FileLink = ({ file }) => {
  ...
  return (
    <tr>
      ...
      <td>
        <Link target="_blank" to={`${file.links.space}/files`}>
      ...
      {file.origin && 
        typeof file.origin === 'object' ? 
          ...
          <td>
            <Link target='_blank' to={`/home/jobs/${file.links.origin_object.origin_uid}`}>
...
HomeFilesSpacesTable.propTypes = {
...
HomeFilesSpacesTable.defaultProps = {
  files: [],
  sortHandler: () => { },
  filters: {},
  toggleFileCheckbox: () => { },
...
const mapStateToProps = (state) => ({
  isFetching: homeFilesSpacesIsFetchingSelector(state),
  isCheckedAll: homeFilesSpacesIsCheckedAllSelector(state),
  filters: homeFilesSpacesFiltersSelector(state),
  path: homePathSpacesSelector(state),
})

const mapDispatchToProps = (dispatch) => ({
  toggleAllFilesCheckboxes: () => dispatch(toggleAllFilesSpacesCheckboxes()),
  toggleFileCheckbox: (id) => dispatch(toggleFileSpacesCheckbox(id)),
})

export default connect(mapStateToProps, mapDispatchToProps)(HomeFilesSpacesTable)

export {
  HomeFilesSpacesTable,
}
