...
import {
  homeWorkflowsIsFetchingSelector,
  homeWorkflowsIsCheckedAllSelector,
  homeWorkflowsFiltersSelector,
...
const HomeWorkflowsTable = ({ workflows, isFetching, isCheckedAll, toggleAllWorkflowsCheckboxes, toggleWorkflowCheckbox, filters, handleFilterValue }) => {
        ...
        <Table>
          <Thead>
            <th className='pfda-padded-l10'>
              <Icon onClick={toggleAllWorkflowsCheckboxes} icon={checkboxClasses} />
            </th>
                ...
                workflows.map((workflow) => {
                  return(
                    <Row
                      workflow={workflow}
                      key={workflow.id}
                      toggleWorkflowCheckbox={toggleWorkflowCheckbox}
                    />)}) : null
              }
            </>
          </Tbody>
...
HomeWorkflowsTable.defaultProps = {
  workflows: [],
  sortHandler: () => { },
  filters: {},
  toggleWorkflowCheckbox: () => { },
...
const mapStateToProps = (state) => ({
  isFetching: homeWorkflowsIsFetchingSelector(state),
  isCheckedAll: homeWorkflowsIsCheckedAllSelector(state),
  filters: homeWorkflowsFiltersSelector(state),
})

const mapDispatchToProps = (dispatch) => ({
  toggleAllWorkflowsCheckboxes: () => dispatch(toggleAllWorkflowsCheckboxes()),
  toggleWorkflowCheckbox: (id) => dispatch(toggleWorkflowCheckbox(id)),
})

export default connect(mapStateToProps, mapDispatchToProps)(HomeWorkflowsTable)
