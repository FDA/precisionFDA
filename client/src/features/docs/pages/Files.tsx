...
export const Files = () => (
  <DocBody>
    ...
    <DocsTip>
      <span className="fa fa-lightbulb-o" aria-hidden="true" /> <strong>TIP:</strong> Web browsers have limitations with regards to their upload capabilities. Therefore, you should only use the browser-based method for small files. Read below for additional options if you have large data to upload.
    </DocsTip>
    ...
    <p>For large files that are on your computer, we suggest using the <strong>precisionFDA CLI</strong>, a command-line tool capable of uploading with multiple threads and more resilient to transient errors.
      The precisionFDA CLI can be used for both files and app assets, and it's available through the <Link to="/assets/new">Create Assets</Link> page.</p>

    <p>Visit that page and download the precisionFDA CLI for your operating system and architecture by clicking the respective button under "Step 3".</p>
    ...
    <p>Run <code>./pfda upload-file --key KEY --file /path/to/file</code>. This command will upload the file to precisionFDA.</p>
    <DocsTip>
      <span className="fa fa-lightbulb-o" aria-hidden="true" /> <strong>TIP:</strong> The CLI saves your key in <code>$HOME/.pfda_config</code>, so after you have run it once, you don't need to specify the key in subsequent invocations.
    </DocsTip>
    <p>The CLI can do a lot more than uploading assets, please see the <Link to="/docs/cli">Command Line Interface documentation page</Link></p>
    ...
    <h2>Examining file origin</h2>

    <p>The system automatically records the origin of files. The "origin" column describes how each file got introduced to precisionFDA; it is set to "Uploaded" if the file was directly uploaded; otherwise it points to a <Link to="/docs/apps#job-details" aria-label='Navigation to job details section in Apps module'>job</Link> if the file was generated by running an app. Further information can be obtained using the <Link to="/docs/tracking">tracking feature</Link>.</p>

    <h2>File details</h2>

    <p>Clicking on a filename in the files list (or on other file links that appear in certain precisionFDA areas) takes you to the file details page, which shows information such as filename, unique id, size, creation time, and origin. In addition, if the file has been used as input to any <Link to="/docs/comparisons">comparisons</Link> or has been attached to any <Link to="/docs/notes">notes</Link>, those will also be visible in respective sub-sections.</p>
    ...
    <DocTable>
      ...
      <tbody>
        <tr>
          <td>Attach to...</td>
          <td>Attach the file to a note. See the <Link to="/docs/notes">notes</Link> section for more info.</td>
        </tr>
        <tr>
          <td>Open</td>
        ...
        <tr>
          <td>Track</td>
          <td>Display a graph tracking the file's provenance. See <Link to="/docs/tracking">Tracking</Link> for more info.</td>
        </tr>
        <tr>
          <td>Publish</td>
          <td>Publicly contribute the file to the precisionFDA community. See <Link to="/docs/publishing">Publishing</Link> for more info.</td>
        </tr>
        <tr>
          <td>Delete</td>
