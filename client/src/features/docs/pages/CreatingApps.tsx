...
import { DocBody, DocTable, DocsTip, VideoWrapper, DocCallout } from '../styles'
...
export const CreatingApps = () => (
  <DocBody>
    ...
    <h2 id="dev-name">App naming conventions</h2>
    ...
    <h2 id="dev-io">Input and Output spec</h2>
    ...
    <h2 id="dev-vm-env">VM Environment</h2>
    ...
    <p>
      The operating system of the virtual machine depends on your selection (either Ubuntu 14.04 or Ubuntu 16.04), with several
      preinstalled packages:
    </p>
    <ul>
      <li><a href="/assets/ubuntu-14-packages.txt">Ubuntu 14.04 preinstalled packages</a></li>
      <li><a href="/assets/ubuntu-16-packages.txt">Ubuntu 16.04 preinstalled packages</a></li>
    </ul>
    <p>
      If your app requires additional Ubuntu packages,
      you can specify so in the "VM Environment" tab. For example, if your app needs Java, we recommend
      adding the "openjdk-7-jre-headless" package. If you are unsure as to what a certain package is called,
      you can use the <a href="http://packages.ubuntu.com">packages.ubuntu.com</a> website to locate
      packages (make sure to select the "trusty" distribution in the search form, as that is
      the codename for Ubuntu 14.04 or "xenial" for Ubuntu 16.04). Note that, specifically for Java 8, we support additional
      packages (such as "openjdk-8-jre-headless") which are not listed on the Ubuntu packages website.
    </p>
    ...
    <DocTable>
      ...
      <tbody>
        <tr>
          <td>1</td>
          <td>A new virtual machine with selected Ubuntu release and preinstalled packages is initialized.</td>
        </tr>
        <tr>
          <td>2</td>
    ...
    <h2 id="dev-instance-types">Available instance types</h2>
    ...
    <h2 id="dev-assets">App assets</h2>
    ...
    <h2 id="dev-own-assets">Your own assets</h2>
    ...
    <pre>
      This asset provides the `bgzip` and `tabix` executables.<br /><br />

      Include this asset if your app needs to compress and index<br />
      a VCF file.<br /><br />

      ### Example usage<br /><br />

      The following produces `file.vcf.gz` and `file.vcf.tbi`:<br /><br />

      ```<br />
      bgzip file.vcf<br />
      tabix -p vcf file.vcf.gz<br />
      ```<br /><br />

      ### Links<br /><br />

      http://www.htslib.org/doc/tabix-1.2.1.html<br />
    ...
    <DocsTip>
      <span className="fa fa-lightbulb-o" aria-hidden="true" /> <strong>TIP:</strong> The uploader saves your key in <code>$HOME/.pfda_config</code>, so after you have run it once, you don't need to specify the key in subsequent invocations.
    </DocsTip>
    ...
    <h2 id="dev-script">App script</h2>
    ...
    <DocCallout>
      ...
      <pre>bedtools slop -i "$intervals_path" -g grch37.chrsizes -b "$padding" &gt;"$intervals_prefix".padded.bed<br />
        emit padded_intervals "$intervals_prefix".padded.bed</pre>
    </DocCallout>



    <h2 id="dev-bash">Bash tips</h2>
    ...
    <h2 id="dev-fork">Forking an app</h2>
    ...
    <h2 id="dev-import">App Import</h2>

    <p>
      On precisionFDA, there are two methods for building apps.
      You may create apps on precisionFDA using the UI, as is described in <Link to="/docs/creating-apps#dev" aria-label="Navigation to creating apps module">Creating Apps</Link>.
      However, advanced users may choose to instead create their app by writing a CWL or WDL file that contains all the pertinent details for the app and uploading this to precisionFDA.
    </p>

    <h2 id="dev-import-reasons">Why Use App Import?</h2>
    <div className="bs-docs-section">
      <p>This import method for creating precisionFDA apps offers several advantages over the step-by-step UI method:</p>
      <ul>
        <li>If you are familiar with CWL or WDL, you may specify all of the parameters for your app without needing to click through multiple UI screens.</li>
        <li>Importing an app via a CWL or WDL script is an easy way to make use of public Docker images in an application format on precisionFDA.</li>
        <li>A CWL or WDL app script may be easily shared with other collaborators, allowing them to build and customize their own versions of this app, either on or off precisionFDA.</li>
        <li>Because the CWL or WDL script contains all parameters used by the app, it can be shared with collaborators who do not have precisionFDA accounts to explain how an analysis was performed.</li>
      </ul>
    </div>
    ...
    <p>
      First, the inputs and outputs specified in the CWL script must be one of the five data types used on precisionFDA - <strong>file, string, float, integer, or Boolean</strong>.
    ...
    <pre>
      #!/usr/bin/env cwl-runner<br /><br />

      class: CommandLineTool<br /><br />

      id: "cgp-chksum"<br /><br />

      label: "CGP file checksum generator"<br /><br />

      cwlVersion: v1.0<br /><br />

      doc: |<br />
      ![build_status](https://quay.io/repository/wtsicgp/dockstore-cgp-chksum/status)<br />
      A Docker container for producing file md5sum and sha512sum. See the [dockstore-cgp-chksum](https://github.com/cancerit/dockstore-cgp-chksum) website for more information.<br /><br />

      requirements:<br />
      - class: DockerRequirement<br />
      dockerPull: "quay.io/wtsicgp/dockstore-cgp-chksum:0.1.0"<br /><br />

      inputs:<br />
      in_file:<br />
      type: File<br />
      doc: "file to have checksum generated from"<br />
      inputBinding:<br />
      position: 1<br /><br />

      post_address:<br />
      type: ["null", string]<br />
      doc: "Optional POST address to send JSON results"<br />
      inputBinding:<br />
      position: 2<br /><br />
      ...
      type: File<br />
      outputBinding:<br />
      glob: check_sums.json<br /><br />

      post_server_response:<br />
      type: ["null", File]<br />
      outputBinding:<br />
      glob: post_server_response.txt<br /><br />

      baseCommand: ["/opt/wtsi-cgp/bin/sums2json.sh"]<br />
    </pre>

    <h2 id="cwl-import">How to Import a CWL File</h2>

    <p>
      To import your CWL file and create a precisionFDA app, you begin by navigating to the Apps page, where you select <strong>Create App</strong>.
      On the Create App page, you can see a button labeled <strong>Import from .cwl file</strong>.
    </p>

    <img width="100%" src={cwlwdlcreateapp} alt="Create an App with import options for CWL,WDL files and fields for App name and title" />

    <p>
      When you click on this button, you'll be prompted to select a local file from your computer with a .cwl extension.
      There is also a textbox to type in the CWL script if you don't have the file ready.
    </p>

    <img width="100%" src={importcwlmodal} alt="Import CWL modal popup with fields for typing CWL and an option to select CWL file" />
    ...
    <img width="100%" src={importcwlmodalscript} alt="Example CWL script on Import CWL modal popup" />

    <p>
      If you see a red error bar at the top of the page, there may be one or more components missing from your CWL script, or there may be a non-authorized data type specified in the script. If your type in your CWL script, make your that your script has correct indentation.
      Make sure that all components are present in the CWL script; you may consult the example CWL script in this help.
    </p>

    <img width="100%" src={importcwlmodalerror} alt="CWL import script error notification for nonexistent base command" />

    <p>
      Click "Import" to finish importing the script. If the script imports successfully, you will see the ID of your new app.
    </p>

    <img width="100%" src={importcwlwdlmodalsuccess} alt="Modal popup notification for successful app creation and App URL" />
    ...
    <img width="100%" src={importcwlio} alt="CWL App inputs and outputs displayed under SPEC tab" />

    <p>
      Documentation information appears in the Readme section.
    </p>

    <img width="100%" src={scripttabs} alt="CWL App inputs and outputs displayed under SPEC tab" />
    ...
    <pre>
      task bwa_mem_tool {'{'}<br />
      &nbsp;&nbsp;Int threads<br />
      &nbsp;&nbsp;Int min_seed_length<br />
      &nbsp;&nbsp;Int min_std_max_min<br />
      &nbsp;&nbsp;File reference<br />
      &nbsp;&nbsp;File reads<br /><br />
      &nbsp;&nbsp;command {'{'}<br />
      &nbsp;&nbsp; &nbsp;&nbsp;bwa mem -t ${'{'}threads{'}'} \<br />
      &nbsp;&nbsp; &nbsp;&nbsp;-k ${'{'}min_seed_length{'}'} \<br />
      &nbsp;&nbsp; &nbsp;&nbsp;-I ${'{'}sep=',' min_std_max_min+{'}'} \<br />
      &nbsp;&nbsp;&nbsp;&nbsp;${'{'}reference{'}'} \<br />
      &nbsp;&nbsp;&nbsp;&nbsp;${'{'}sep=' ' reads+{'}'} &gt; output.sam<br />
      &nbsp;&nbsp;{'}'}<br />
      &nbsp;&nbsp;output {'{'}<br />
      &nbsp;&nbsp;&nbsp;&nbsp;File sam = "output.sam"<br />
      &nbsp;&nbsp;{'}'}<br />
      &nbsp;&nbsp;runtime {'{'}<br />
      &nbsp;&nbsp;&nbsp;&nbsp;docker: "broadinstitute/baseimg"<br />
      &nbsp;&nbsp;{'}'}<br />
      {'}'}
    </pre>
    ...
    <img width="100%" src={importwdlmodal} alt="Import WDL modal popup with fields for typing WDL and an option to select WDL file" />
    ...
    <img width="100%" src={importwdlmodalscript} alt="Example WDL script on Import WDL modal popup" />
    ...
    <img width="100%" src={importwdlmodalerror} alt="WDL import script error notification for wrong docker image name format" />
    ...
    <img width="100%" src={importwdlio} alt="Modal popup notification for successful app creation and App URL" />
    ...
    <ul>
      <li>
        ...
        <img width="100%" src={wdldefaultvals} alt="Requried and optional inputs under SPEC tab" />
        <p>
        ...
        <img width="100%" src={wdlioedit} alt="WDL I/O Spec tab with Class,Name,Label,Helptext,DefaultValue,Optional field labels" />
        <p>
          Default values are shown in spec after editing.
        </p>
        <img width="100%" src={wdldefaultvals2} alt="Requried and optional inputs under SPEC tab" />
      </li>
      <li>
        ...
        <img width="100%" src={wdljsoninputs} alt="Script in inputs.json file" />
        <p>
          Modify the script for the optional inputs as following, make sure to change the workflow name (run_app) and the app name (app) into your workflow name and your app name in this script.
        </p>
        <img width="100%" src={wdljsoninputs2} alt="Modifiled script for optional inputs in inputs.json file" />
        <p>
