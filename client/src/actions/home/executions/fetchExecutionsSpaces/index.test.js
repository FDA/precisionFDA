...
import fetchExecutionsSpaces from './index'
...
describe('fetchExecutionsSpaces()', () => {
  afterEach(() => {
    fetchMock.reset()
  })
  ...
  describe('dispatch actions', () => {
    ...
    const url = '/api/jobs/spaces?page=1'
    ...
    it('dispatches correct actions on success response', () => {
      ...
      return store.dispatch(fetchExecutionsSpaces()).then(() => {
        const actions = store.getActions()

        expect(actions).toEqual([
          { type: HOME_EXECUTIONS_FETCH_START, payload: HOME_ENTRIES_TYPES.SPACES },
          { type: HOME_EXECUTIONS_FETCH_SUCCESS, payload: { executionsType: HOME_ENTRIES_TYPES.SPACES, executions, pagination }},
        ])
      })
    })
    ...
    it('dispatches correct actions on failure response', () => {
      ...
      return store.dispatch(fetchExecutionsSpaces()).then(() => {
        ...
        expect(actions).toEqual([
          { type: HOME_EXECUTIONS_FETCH_START, payload: HOME_ENTRIES_TYPES.SPACES },
          { type: HOME_EXECUTIONS_FETCH_FAILURE, payload: HOME_ENTRIES_TYPES.SPACES },
