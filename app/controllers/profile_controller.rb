    ...
    Axlsx::Package.new do |p|
      ...
      Time.use_zone ActiveSupport::TimeZone.new('America/New_York') do
        ...
        p.workbook.add_worksheet(:name => "Requests") do |sheet|
          sheet.add_row ["time", "in_system?", "first name", "last name", "email", "organization", "self-represent?", "address", "phone", "duns", "consistency challenge?", "research?", "clinical?", "has data?", "has software?", "reason" ]
          Invitation.includes(:user).find_each do |inv|
            ...
            row += [inv.first_name, inv.last_name, inv.email, inv.org, inv.singular, inv.address, inv.phone, inv.duns, inv.giab_challenge_intent, inv.research_intent, inv.clinical_intent, inv.req_data, inv.req_software, inv.req_reason]
            sheet.add_row row
          end
        end
