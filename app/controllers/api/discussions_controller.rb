module Api
  ...
  class DiscussionsController < ApiController
    ...
    def create
      response = https_apps_client.discussion_create(params[:discussion])
      render json: response, adapter: :json
    end

    # PUT /api/discussions
    def update
      response = https_apps_client.discussion_update(params[:id], params[:discussion])
      render json: response, adapter: :json
    end
    ...
    def answer_update
      response = https_apps_client.answer_update(params[:discussion_id], params[:answer_id], params[:answer])
      render json: response, adapter: :json
    end
    ...
    def answer_create
      response = https_apps_client.answer_create(params[:discussion_id], params[:answer])
      render json: response, adapter: :json
    end

    # POST /api/discussions/:id/answers/:answer_id/publish
    def answer_publish
      response = https_apps_client.answer_publish(params[:discussion_id], params[:answer])
      render json: response, adapter: :json
    end

    # POST /api/discussions/:id/comments
    def discussion_comment_create
      response = https_apps_client.discussion_comment_create(params[:id], params[:comment])
      render json: response, adapter: :json
    end
