class AppsController < ApplicationController
  ...
  def featured
    org = Org.featured
    if org
      @apps_grid = initialize_grid(AppSeries.accessible_by_public.includes(:user, :taggings).
  ...
  def show
    ...
    @revisions = @app.app_series.accessible_revisions(@context).select(:title, :id, :uid, :revision, :version)
    @notes = @app.notes.real_notes.accessible_by(@context).
      order(id: :desc).
      page unsafe_params[:notes_page]
    @answers = @app.notes.accessible_by(@context).
      answers.
      order(id: :desc).
      page unsafe_params[:answers_page]
    @discussions = @app.notes.accessible_by(@context).
      discussions.
      order(id: :desc).
  ...
  def validate_app_before_export
    ...
    if @app.assets.accessible_by(@context).count != @app.assets.count
      flash[:error] = "This app cannot be exported because"\
        "one or more assets are not accessible by you."
      redirect_to app_path(@app)
      return
    end
    if @app.assets.any? { |a| a.license.present? && !a.licensed_by?(@context) }
      flash[:error] = "This app contains one or more assets which need to be licensed."\
              "Please run the app first in order to accept the licenses."
      redirect_to app_path(@app)
      nil
    end
  end
  ...
  def js_info(app, challenges)
    {
      app: (begin
                app.slice(:id, :dxid, :title, :readme, :revision, :dev_group, :release).
