class AppsController < ApplicationController
  ...
  def export
    ...
    cmds = []
    cmds << "# Generated by precisionFDA on #{Time.now.to_s}"
    cmds << "# The links and authorization tokens in this file are valid only for 24h."
    cmds << ""
    cmds << "# Exported app: #{app.name}, authored by: #{app.user.username}"
    cmds << ""
    ...
    cmds << "RUN curl https://dl.dnanex.us/F/D/y6X31xzzY03vGp03v8ypx6jV83GZ4pg0P1VbfJ0Z/run-1.0.tar.gz | tar xzf - -C /usr/bin/ --no-same-owner --no-same-permissions"
    cmds << ""
