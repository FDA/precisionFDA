$(document).on('page:change', function () {
  if ($("body.pfda-controller-files.pfda-action-new").length == 0) {
    return;
  }
  ...
  $("#upload-button").click(function() {
    $("#upload-button").hide();
    $("#upload-input").hide();
    uploadFiles($("#upload-input")[0].files);
  });
  ...
  function uploadFile(file, cb) {
    ...
    var chunk_size = 104857600;
    var num_chunks = Math.ceil(size * 1.0 / chunk_size);
    if (num_chunks > 10000) {
      chunk_size = Math.ceil(size * 1.0 / 10000);
      num_chunks = Math.ceil(size * 1.0 / chunk_size);
    }

    apiCall("/api/create_file", {"name": file.name}, function(res) {
      var id = res.id;

      function uploadChunks(index, offset) {
        var this_chunk_size = chunk_size;
        if (offset + this_chunk_size > size) {
          this_chunk_size = size - offset;
        }
        calculateMD5(file, offset, this_chunk_size, function(md5) {
          apiCall("/api/get_upload_url", {"id": id, "index": index, "size": this_chunk_size, "md5": md5}, function(res) {
      ...
      function closeFile() {
        apiCall("/api/close_file", {"id": id}, function(res) {
          cb();
        });
      }
  ...
  //Spark MD5
  function calculateMD5(file, offset, len, cb) {
    ...
    function loadNext() {
      var start = offset + currentChunk * chunkSize,
          end = (((currentChunk + 1) * chunkSize) >= len) ? offset + len : start + chunkSize;
