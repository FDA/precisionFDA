    ...
    for own analysis_id, jobs of @params.analyses_jobs
      ...
      for job in jobs
        ...
        rows = [
          ...
          $execution = $("<td>").append($("<a>").attr("href", "/jobs/#{job.uid}").html(job.execution).prepend($("<span>").addClass("fa #{job.icon} fa-fw"))),
          $app = $("<td>").append($("<a>").attr("href", "/apps/#{job.app_uid}").html(" #{job.app_title}").prepend($("<span>").addClass("fa fa-cube fa-fw")))
