...
RUN apt-get update
RUN apt-get -y -qq install libgtk-3-dev
...
RUN wget -nv -O /tmp/firefox.tar.bz2 https://ftp.mozilla.org/pub/firefox/releases/${FIREFOX_VERSION}/linux-x86_64/en-US/firefox-${FIREFOX_VERSION}.tar.bz2
...
# Geckodriver
RUN wget -nv https://github.com/mozilla/geckodriver/releases/download/v0.19.1/geckodriver-v0.19.1-linux64.tar.gz
RUN apt-get -y -qq install software-properties-common python-software-properties
...
ENTRYPOINT $APP_DIR/docker/ui_test.entrypoint.sh
