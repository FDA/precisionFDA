...
def filter_dict(dict_, excl_keys):
    return {k: v for k, v in dict_.items() if k not in excl_keys}
...
def get_job_input_filenames(job_input_file):
    ...
    for input_name, value in job_input.items():
    ...
    rest_hash = {key: val for key, val in job_input.items() if key not in files}
...
def gen_bash_vars(job_input_file, job_homedir=None, check_name_collision=True):
    ...
    def gen_text_line_and_name_collision(key, val, really_check=True):
        ''' In the absence of a name collision, create a line describing a bash variable.
        '''
        if check_name_collision and really_check:
    ...
    for key, desc in rest_hash.items():
        gen_text_line_and_name_collision(key, string_of_value(desc))
    for file_key, desc in file_key_descs.items():
        gen_text_line_and_name_collision(file_key, string_of_value(desc['handler']), False)
        ...
        gen_text_line_and_name_collision(file_key + "_path", string_of_value(desc['path']), False)
