...
RSpec.describe Workflow::Stages::InputPresenter, type: :model do
  ...
  context "when the input has incorrect compatible linking" do
    before do
      input["requiredRunInput"] = false
      presenter.valid?
    end
    it "add errors to the attribute" do
      expect(presenter.errors[:values])
        .to include(I18n.t("values.compatible_linking", locale_options))
    end
  end

  context "when the input doesn't have link to another stage output" do
    let!(:slot) { params["slots"].second }
    let(:slot_number) { 1 }

    before do
      input["values"]["name"] = nil
      presenter.valid?
    end
    it "add errors to the attribute" do
      expect(presenter.errors[:values])
        .to include(I18n.t("values.stage_output_linking", locale_options))
    end
  end

  context "when the input has wrong output linking" do
    let!(:slot) { params["slots"].second }
    let(:slot_number) { 1 }
    let(:output) { params["slots"].first["outputs"].first }
    let(:output_spec) do
      { params["slots"].first["slotId"] => { output["name"] => output["class"] } }
    end

    before { allow(slot_presenter).to receive(:output_classes).and_return(output_spec) }

    context "and has mismatch with slot_id of previous stage" do
      before do
        input["values"]["id"] = "mismatch_id"
        presenter.valid?
      end
      it "add errors to the attribute" do
        expect(presenter.errors[:values])
          .to include(I18n.t("values.wrong_output_stage_linking", locale_options))
      end
    end

    context "and is not linked with previous stage" do
      let(:output) { params["slots"].first["outputs"].second }

      before { presenter.valid? }
      it "add errors to the attribute" do
        expect(presenter.errors[:values])
          .to include(I18n.t("values.wrong_output_stage_linking", locale_options))
      end
    ...
    context "and is not linked with previous stage" do
      let(:output_spec) { { params["slots"].first["slotId"] => { output["name"] => "string" } } }

      before { presenter.valid? }

      it "add errors to the attribute" do
        options = locale_options.merge(input_class: presenter.input_class, output_class: "string")
        expect(presenter.errors[:values])
          .to include(I18n.t("values.wrong_output_class_linking", options))
      end
    end
  end
end
