...
RSpec.describe Workflow::Cwl::InputParser, type: :model do
  ...
  let!(:second_app) do
    create(:app, dxid: params["slots"].second["uid"].split("-1").first, user_id: user.id, spec: app_spec)
  end
  ...
  context "when step is first" do
    context "and link name is wrong" do
      before do
        input_name = step_parser.step_json["in"].keys.second
        step_parser.step_json["in"][input_name] = "wrong_link"
        parser.valid?
    ...
    context "and link type is wrong" do
      before do
  ...
  context "when step is not_first" do
    ...
    context "and link is output of previous step" do
      it { is_expected.to be_valid }
    end
