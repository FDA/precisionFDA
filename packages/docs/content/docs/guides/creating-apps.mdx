...
For our [example](/guides/creating-apps#input-and-output-spec), the system would define the following variables:
...
On precisionFDA, there are two methods for building apps. You may create apps on precisionFDA using the UI, as is described in the [Apps and Workflows tutorial](/tutorials/apps/introduction). However, advanced users may choose to instead create their app by writing a CWL or WDL file that contains all the pertinent details for the app and uploading this to precisionFDA.
