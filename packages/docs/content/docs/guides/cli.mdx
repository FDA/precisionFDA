...
<div className="flex gap-4 shrink">
  <a href="https://pfda-production-static-files.s3.amazonaws.com/cli/pfda-linux-2.15.1.tar.gz" className='no-underline' target="_blank" rel="noreferrer">
  ...
  <a href="https://pfda-production-static-files.s3.amazonaws.com/cli/pfda-darwin-2.15.1.tar.gz" className='no-underline' target="_blank" rel="noreferrer">
  ...
  <a href="https://pfda-production-static-files.s3.amazonaws.com/cli/pfda-windows-2.15.1.zip" className='no-underline' target="_blank" rel="noreferrer">
...
Alternatively, visit the [**Assets**](https://precision.fda.gov/home/assets) page and click on the 'Generate CLI Key' button and copy the presented key.
...
> **Result Format:**
>
> Some CLI commands support a result response in JSON format. Simply append the `-json` flag to the command. For single-item commands, if an error occurs it results in a JSON object with one key, **"error"**. The batch commands `mkdir`, `rm`, `rmdir`, `upload-file`, and `download` always emit a single parseable JSON array - including when the command fails before processing any item, for example on an invalid argument. Each item is either a result object or an error object with one key, **"error"**, and any error item makes the command exit with a non-zero code. Each uploaded file is reported as **"id"**, **"name"**, and **"url"**; a folder upload adds a final item with the **"url"** of the created folder.
>
> **Debug Mode:**
>
> Append the `-debug` flag to any command to print a trace of every HTTP request and response (URL, headers, bodies, status, duration) to stderr. Your authorization key never appears in the trace: secret-bearing headers are omitted, and signatures/tokens are masked in the request-line URL and the `Location` redirect header. Non-JSON bodies (file downloads, binary upload chunks) are never dumped. **JSON request and response bodies, however, are printed verbatim (up to 10KB, after which request bodies are omitted and response bodies are truncated) and are not scrubbed - so any presigned upload/download URLs the API returns inside them, including their signatures and access tokens, appear in the trace in full.** When contacting support about a failing command, re-run it as `pfda <command> -debug 2>debug.log` and attach the log, but always review it first before sharing.
>
> **Version and FIPS Status:**
>
> Run `pfda -version` to print CLI build information, Go version, TLS version, and native Go FIPS 140-3 status. Distribution builds use Go's `GOFIPS140` support and print the selected Go Cryptographic Module version when FIPS mode is active.
...
- `-h, -help`: Displays the help message and exits.
- `-name <NAME>`: The asset name. Required. Must end in `.tar` or `.tar.gz`.
- `-root <PATH>`: Path to the folder with the asset contents. Required.
- `-readme <FILE>`: Path to the readme file. Required. Must end in `.md` or `.txt`.
- `-json`: Displays the command result in JSON format.
...
- `-r, -recursive`: Recursively downloads the content of the selected folder.
- `-json`: Displays the command result in JSON format.
...
The CLI command `pfda rmdir` is designed for removing folders from My Home or a Space. All you need is the unique folder ID. For safety reasons, only empty folders are removed by default - use the `-recursive` flag to remove a folder together with all of its contents.
...
- `-h, -help`: Displays the help message and exits.
- `-r, -recursive`: Recursively removes the folder together with all of its contents.
- `-json`: Displays the command result in JSON format.
...
_# Removes a folder and all of its nested contents._<br/>
`./pfda rmdir 2704 -recursive`
...
The CLI command `pfda rm-discussion` is designed for removing a discussion by its unique numeric ID. The discussion's replies and attachments are removed along with it.

**Please be cautious. This is a permanent, destructive operation and cannot be undone.**

### Usage

`./pfda rm-discussion <DISCUSSION_ID> [...FLAG]`
...
The following examples demonstrate use cases for the `rm-discussion` command:

_# Removes the discussion with ID 15._<br/>
`./pfda rm-discussion 15`

_# Removes the discussion with ID 15 and responds with the JSON result._<br/>
`./pfda rm-discussion 15 -json`

> [!info] Tips and Notes
> **Finding DISCUSSION ID:**
>
> To find a DISCUSSION_ID, inspect the URL when viewing a discussion. For example, in the URL **"https://precision.fda.gov/spaces/1995/discussions/15"**, the Discussion ID is **15**.
> You can also use the precisionFDA CLI command `pfda ls-discussions -space-id <SPACE_ID>` to list discussions in a space.
>
> **Permissions:**
>
> You can only remove a discussion if you have permission to edit it (typically as its author or with the appropriate role in the containing space).

## Removing Discussion Replies

The CLI command `pfda rm-reply` is designed for removing a single discussion reply (answer or comment) by its unique numeric ID.

**Please be cautious. This is a permanent, destructive operation and cannot be undone.**

### Usage

`./pfda rm-reply <REPLY_ID> [...FLAG]`
...
The following examples demonstrate use cases for the `rm-reply` command:

_# Removes the reply with ID 25._<br/>
`./pfda rm-reply 25`

_# Removes the reply with ID 25 and responds with the JSON result._<br/>
`./pfda rm-reply 25 -json`

> [!info] Tips and Notes
> **Reply IDs:**
>
> A reply can be either an **answer** or a **comment**; both share the same numeric ID space and can be removed with this command.
>
> **Permissions:**
>
> You can only remove a reply if you have permission to edit it (typically as its author or with the appropriate role in the containing space).
*/}
...
- `-h, -help`: Displays the help message and exits.
- `-space-id <ID>`: Lists assets in the specified space.
...
_# Lists all assets from the space with ID 24._<br/>
`./pfda ls-assets -space-id 24`
...
The CLI command `pfda ls-discussions` is designed for listing discussions in a given space.

### Usage

`./pfda ls-discussions -space-id <SPACE_ID>`

### Available Flags

- `-h, -help`: Displays the help message and exits.
- `-space-id <ID>`: Lists discussions in the specified space. Required to target a space.
- `-json`: Displays the command result in JSON format.
...
- `-protected`: Shows only protected spaces.
- `-json`: Displays the command result in JSON format.
...
`./pfda ls-members -space-id <SPACE_ID>`

### Available Flags

- `-h, -help`: Displays the help message and exits.
- `-space-id <ID>`: Lists members in the specified space. Required to target a space.
- `-json`: Displays the command result in JSON format.
...
The CLI command `pfda describe` is designed to display details about an entity. All you need is the unique entity ID. Supported types are: app, discussion, file, folder, execution, and workflow.

For most entities the ID is of the form `{entity}-ABCDEF1234567890-{number}` (for example `app-…`, `file-…`, `workflow-…`). Two cases are worth noting: an **asset** is referenced by its underlying `file-…` ID, and an **execution** is referenced by its `job-…` ID. A **discussion** uses a short numeric form instead, e.g. `discussion-17`.
...
The CLI commands `pfda cat` and `pfda head` are designed to print the content of a file. They accept a single parameter: the unique file ID. Use `cat` to print the whole file's content, or `head` to print just the first few lines.

### Usage

`./pfda cat <FILE_ID>`

`./pfda head <FILE_ID> [...FLAG]`

### Available Flags

- `-h, -help`: Displays the help message and exits.
- `-lines <NUMBER>`: Alters the number of lines to display (for the `head` command only). Defaults to 10.
...
|---|---|
| Version 2.15.1 (08/06/2026) | Fixed issue with executable build |
| Version 2.15.0 (08/04/2026) | Reworked `--json` output and exit-code semantics for batch commands |
| Version 2.14.2 (08/03/2026) | Fixed issue with the distribution build across all platforms |
| Version 2.14.1 (07/27/2026) | Fixed issue with upload-asset across all platforms |
| Version 2.14.0 (07/27/2026) | New features: rmdir -r flag for deleting non-empty folders, -debug flag for requests tracing |
| Version 2.13.0 (06/30/2026) | {/* New features: rm-discussion, rm-reply; */}Bug fixes and improvements; upgraded Go to 1.26.4 |
