...
job=$(
  curl -s localhost:$GSRS_PORT/api/v1/scheduledjobs -H "AUTHENTICATION_USERNAME: admin" |
...
run_result=$(curl -s "$job_execute_url" -H "AUTHENTICATION_USERNAME: admin")
...
while true ; do
  job=$(curl -s -H "AUTHENTICATION_USERNAME: admin" $job_url)
