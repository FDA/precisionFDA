...
RSpec.describe Api::WorkflowsController, type: :controller do
  ...
  describe "POST copy" do
    ...
    let(:copy_service) { instance_double(CopyService::WorkflowCopier, copy: {}) }

    before do
      authenticate!(user)

      allow(CopyService::WorkflowCopier).to receive(:new).and_return(copy_service)
    end

    it "copies workflows" do
      post :copy, params: { item_ids: workflows.map(&:id), scope: space.scope, properties: { createAppSeries: true } }, format: :json

      expect(response).to be_successful

      workflows.each do |wf|
        expect(copy_service).to have_received(:copy).with(wf, space.scope, { createAppSeries: "true" }).exactly(1).times
      end
    end
  end
