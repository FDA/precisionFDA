class QueriesController < ApplicationController
  def create
    if request.post?
      ...
      redirect_to truth_challenges_path(unsafe_params[:page_tab])+"?query_id=#{@query.id}"
    end
  end

  def destroy
    sq = SavedQuery.find_by!(id: unsafe_params[:id])
    if sq.user_id == @context.user_id || @context.user.can_administer_site?
      sq.destroy
    end
    # TODO: pass in unsafe_params[:page_tab] instead of fixing tab
