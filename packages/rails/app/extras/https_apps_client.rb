...
class HttpsAppsClient # rubocop:disable Metrics/ClassLength
  ...
  # @param job_uid [String] Job uid to terminate.
  # @param auth_code [String] Code generated by the auth server to authenticate with the workstation
  # @param api_key [String] API key
  def workstation_set_api_key(job_uid, auth_code, api_key)
    request(
      "/jobs/#{job_uid}/setAPIKey",
      {
        code: auth_code,
        key: api_key,
      },
  ...
  def workstation_snapshot(job_uid, auth_code, api_key, name, terminate)
    request(
      "/jobs/#{job_uid}/snapshot",
      {
        code: auth_code,
        key: api_key,
        name: name,
