...
function ComboboxInput({
  ...
  return (
    <ComboboxPrimitive.InputGroup
      render={<InputGroup />}
      className={cn(
        'h-9 min-h-9 w-full min-w-0 bg-transparent has-[[data-slot=input-group-control]:focus-visible]:border-primary has-[[data-slot=input-group-control]:focus-visible]:ring-3 has-[[data-slot=input-group-control]:focus-visible]:ring-primary/30 dark:bg-input/30 dark:hover:bg-input/50',
        className,
      )}
    >
...
function ComboboxContent({
  ...
  alignOffset = 0,
  anchor,
  /** When set (e.g. modal floating host), portaled popups mount here instead of body. */
  container,
  ...props
...
function ComboboxItem({ className, children, ...props }: ComboboxPrimitive.Item.Props) {
  return (
    <ComboboxPrimitive.Item
      ...
      className={cn(
        "relative flex w-full cursor-default items-center gap-2 rounded-sm py-1.5 pr-8 pl-2 text-sm outline-hidden select-none data-highlighted:bg-accent data-highlighted:text-accent-foreground data-selected:bg-primary! data-selected:text-primary-foreground! not-data-[variant=destructive]:data-highlighted:**:text-accent-foreground data-disabled:pointer-events-none data-disabled:opacity-50 [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4 *:[span]:last:flex *:[span]:last:items-center *:[span]:last:gap-2",
        className,
      )}
      {...props}
    >
