...
async function featureRequest<T>(
  ...
  { uids, featured }: { uids: (number | string)[]; featured: boolean },
...
export const useFeatureMutation = <T extends { id: string | number }>({
  resource,
  onSuccess,
}: {
  resource: APIResource
  onSuccess?: (res: RequestResponse<T>) => void
}) => {
  const featureMutation = useMutation({
    ...
    onSuccess: async res => {
      if (res.meta[0].type === 'success') {
        toastSuccess(`Success: ${res.meta[0].message}`)
        if (onSuccess) onSuccess(res)
      } else {
        toastError(`Error: ${res.meta[0].message}`)
      }
    },
    onError: (res: unknown) => {
      console.error('Error featuring resource:', res)
      toastError('Error: featuring')
    },
  })
