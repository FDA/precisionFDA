...
function fileSizeParamMap(fileSize?: {
  ...
  const from =
    fileSize.from !== null && fileSize.from !== undefined && fileSize.from !== '' ? Number(fileSize.from) : null
  const to = fileSize.to !== null && fileSize.to !== undefined && fileSize.to !== '' ? Number(fileSize.to) : null
...
function parseRange(value: string | null): { from: number | null; to: number | null } | undefined {
  ...
  return {
    from: Number.isNaN(min as number) ? null : min,
    to: Number.isNaN(max as number) ? null : max,
  }
}
...
function serializeRange(value: unknown): string | undefined {
...
export function useFilterParams({
  ...
  filters: FilterArgs
  onSetFilter?: (values: Record<string, unknown>) => void
}) {
  ...
  const setFilterParam = useCallback(
    (values: Record<string, unknown>, replace: boolean = true) => {
      ...
      navigate(`${currentUrl.pathname}?${newParams.toString()}`, { replace })
    },
    [filters, navigate],
  )

  const setSearchFilter = useCallback(
    (val: ColumnFiltersState) => {
