...
export const spacesMocks = [
  http.get('/api/spaces/:id', ({ params }) => HttpResponse.json<FetchSpaceDetailsResponse>({ space: spacesById[parseInt(params.id, 10)], meta: {} })),
  http.get('/api/spaces/', () => HttpResponse.json<FetchSpacesListResponse>({
    'spaces': spaces,
    'meta': {
      'count': 20,
      'pagination': {
