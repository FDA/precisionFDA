...
const DiscussionListTable = ({
  ...
  const onClick = (discussion: Discussion) => {
    const spaceId = getSpaceIdFromScope(discussion.scope)
...
export const DiscussionList = ({ scope, canCreateDiscussion }: { scope: HomeScope; canCreateDiscussion?: boolean }) => {
  const { query, selectedIndexes, setSelectedIndexes, saveColumnResizeWidth, colWidths, saveHiddenColumns, setPerPageParam, setPageParam } = useList<ListType>({
