...
import { type JobLogItem, WEBSOCKET_MESSAGE_TYPE, type WebSocketMessage } from '../home/types'
import type { JobState } from './executions.types'
...
export const Logs = ({ jobUid, jobState }: { jobUid: string; jobState: JobState }) => {
  ...
  const previousStatusRef = useRef<'idle' | 'connecting' | 'open' | 'closed'>('idle')
  ...
  const { lastJsonMessage, sendMessage, status } = useWebSocket<WebSocketMessage>(
    getNodeWsUrl(),
    {
  ...
  useEffect(() => {
    if (status !== 'open' || isStreamingDone || !jobUid) {
      return
    }

    sendMessage(JSON.stringify({ event: WEBSOCKET_MESSAGE_TYPE.JOB_LOG, data: { jobUid } }))
  }, [isStreamingDone, jobUid, sendMessage, status])

  useEffect(() => {
    if (lastJsonMessage?.type !== WEBSOCKET_MESSAGE_TYPE.JOB_LOG) {
      return
    }

    const newLog = lastJsonMessage.data as JobLogItem
    if (isStopSignal(newLog)) {
      setIsStreamingDone(true)
      setLogs(prevLogs =>
        prevLogs.length === 0 ? [{ level: 'INFO', msg: 'No logs found', line: 0, source: 'client' }] : prevLogs,
      )
      return
    }
  ...
  useEffect(() => {
    if (previousStatusRef.current === 'open' && status === 'closed' && !isStreamingDone) {
      setLogs([])
    }

    previousStatusRef.current = status
  }, [isStreamingDone, status])
  ...
  }, [logs, jobState, rowVirtualizer.scrollToIndex])
