...
describe('DataPortalService', () => {
  ...
  describe('#create', () => {
    ...
    it('fail without site admin privileges', async () => {
      ...
      try {
        ...
        if (error instanceof Error) {
          ...
          expect(error.message).to.equal('Only site admins can create Data Portals')
        } else {
  ...
  describe('#update', () => {
    ...
    it('fail without space lead privileges', async () => {
      const space = create.spacesHelper.create(em, { name: 'space-name' })
      await em.flush()
      const portal = create.dataPortalsHelper.create(
        em,
      ...
      try {
        ...
        if (error instanceof Error) {
          expect(error.name).to.equal('PermissionError')
          expect(error.message).to.equal('Only portal leads can update portal settings')
        } else {
          expect.fail('Unexpected error type')
        }
      }
    ...
    it('fail without space lead or admin privileges', async () => {
      ...
      try {
        ...
        if (error instanceof Error) {
          expect(error.name).to.equal('PermissionError')
          expect(error.message).to.equal('Only portal admins and leads can update portal content')
        } else {
          expect.fail('Unexpected error type')
        }
      }
  ...
  describe('#get', () => {
    ...
    it('fail with non active membership', async () => {
      const portal = await createPortal(
        'space-name',
        'host-lead',
        'guest-lead',
  ...
  describe('#updateCardImageUrl', () => {
    ...
    it('no corresponding data portal found', async () => {
      findDataPortalsStub.resolves([])
