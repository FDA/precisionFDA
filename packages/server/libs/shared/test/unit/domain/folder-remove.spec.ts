...
describe('remove folder tests', () => {
  ...
  it('test remove folder', async () => {
    ...
    expect(loadedFolderEvent.data).to.equal(
      `{"id":${folder1.id.toString()},"scope":` +
  ...
  it('test fail remove folder in locked verification space', async () => {
    ...
    const folderToDelete = create.filesHelper.createFolder(
      em,
      { user },
      { name: 'folderInSpace', scope: `space-${space.id}` },
    )
    await em.flush()
    ...
    try {
      ...
      expect(error.message).to.equal(
        `You have no permissions to remove ${folderToDelete.name} as` +
  ...
  it('test fail to remove file in space with membership Viewer', async () => {
    ...
    const folderToDelete = create.filesHelper.createFolder(
      em,
      { user: userCreator },
      { name: 'fileInspace', scope: `space-${space.id}` },
    )
    await em.flush()
    ...
    try {
      ...
      expect(error.message).to.equal(`You have no permissions to remove '${folderToDelete.name}'.`)
    }
  })
})
