...
describe('UnlockNodeFacade', () => {
  ...
  const nodeService = {
    ...
    unlockFile: nodeServiceUnlockFileStub,
  ...
  beforeEach(() => {
    ...
    nodeServiceUnlockFileStub.reset()
    nodeServiceUnlockFileStub.throws()
  ...
  describe('#unlockNodes', async () => {
    it('sync', async () => {
      nodeServiceLoadNodesStub.withArgs([file1.id, file2.id], { locked: true }).resolves([file1, file2])
      nodeHelperFilterNodesByUserStub.withArgs([file1, file2]).resolves([file1, file2])
      nodeServiceUnlockFileStub.reset()
      const facade = getInstance()

      await facade.unlockNodes([file1.id, file2.id], false)

      expect(nodeServiceUnlockFileStub.callCount).to.equal(2)
      expect(nodeServiceUnlockFileStub.getCall(0).args[0]).to.equal(file1.id)
      expect(nodeServiceUnlockFileStub.getCall(1).args[0]).to.equal(file2.id)
    })
    ...
    it('async', async () => {
      ...
      await facade.unlockNodes([file1.id, file2.id], true)
      ...
      expect(notificationServiceCreateNotificationStub.getCall(0).args[0]).to.deep.include({
        message: 'Successfully unlocked 2 files',
        userId: userCtx.id,
    ...
    it('unsupported node type', async () => {
      nodeServiceLoadNodesStub.withArgs([file1.id, asset.id], { locked: true }).resolves([file1, asset])
      ...
      await expect(facade.unlockNodes([file1.id, asset.id], true)).to.be.rejectedWith(
        PermissionError,
        `Unlocking of asset  ${asset.uid} is not allowed`,
      )
