...
export class ChallengeUpdateCardImageUrlOperation extends BaseOperation<OpsCtx, number, void> {
  async run(challengeId: number): Promise<void> {
    ...
    if (!challenge) {
      throw new NotFoundError(
        `ChallengeUpdateCardImageUrlOperation: Challenge ${challengeId} not found`,
      )
    }

    if (!challenge.cardImageId) {
      throw new NotFoundError(
        `ChallengeUpdateCardImageUrlOperation: Challenge ${challengeId} has empty card_image_id`,
      )
    }
    ...
    logger.log(
      { challengeId, cardImage },
      `ChallengeUpdateCardImageUrlOperation: Updating ${challengeId} cardImage`,
    )
    if (!cardImage) {
      throw new NotFoundError(
        `ChallengeUpdateCardImageUrlOperation: Cannot find card image id ${challenge.cardImageId} for challenge ${challengeId}`,
      )
    }
