...
export class ChallengeResourceRepository extends BaseEntityRepository<ChallengeResource> {
  async findChallengeResourcesByFileUid(fileUid: string): Promise<ChallengeResource[]> {
    return this.createQueryBuilder('chr').leftJoinAndSelect('chr.userFile', 'r').where({ 'r.uid': fileUid }).getResult()
  }
}
