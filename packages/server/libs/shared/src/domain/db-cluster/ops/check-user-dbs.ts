...
const recreateDbSyncOperationIfMissing = async (dbCluster: DbCluster, log: Logger, userCtx: UserCtx): Promise<void> => {
  log.log(`Checking dbcluster ${dbCluster.dxid}, userId ${userCtx.id}, status ${dbCluster.status}`)
  const dbSyncOperation = await findRepeatable(SyncDbClusterOperation.getBullJobId(dbCluster.dxid))
  if (!dbSyncOperation) {
    log.log({ dbCluster, userId: userCtx.id }, 'Recreating missing DB Cluster sync operation')
...
export class CheckUserDbClustersOperation extends WorkerBaseOperation<UserOpsCtx, never, Maybe<DbCluster[]>> {
  ...
  async run(): Promise<Maybe<DbCluster[]>> {
    ...
    this.ctx.log.log({ nonTerminatedDbClusters, userId }, 'CheckUserDbClustersOperation: Found DB Clusters for userId')
