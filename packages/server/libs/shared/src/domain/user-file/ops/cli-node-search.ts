...
class CLINodeSearchOperation extends BaseOperation<UserOpsCtx, CLINodeSearchInput, Node[]> {
  async run(input: CLINodeSearchInput): Promise<Node[]> {
    ...
    const spaces = await em.find(Space, {
      spaceMemberships: {
        user: {
          id: this.ctx.user.id,
        },
    ...
    if (input.type === 'Folder') {
      ...
      result = await em.find(
        ...
        {
          $or: [
            { $and: [{ user: user.id, scope: STATIC_SCOPE.PRIVATE, parentFolder }] },
            {
              $and: [
                {
                  scope: { $in: spaceScopes as SCOPE[] },
                  scopedParentFolder: scopedParentFolderId,
                },
              ],
            },
          ],
