...
import { getNodePath, getParentFolder } from '@shared/domain/user-file/user-file.helper'
import {
  FILE_STATE_DX,
  FILE_STATE_PFDA,
  FILE_STI_TYPE,
...
export class NodeService {
  ...
  async loadNodes(ids: number[], filters: nodeQueryFilter): Promise<Node[]> {
    ...
    for (const node of nodes) {
      if (node.stiType === FILE_STI_TYPE.FOLDER) {
        await this.collectChildren(node as Folder, wholeTree)
      } else {
        wholeTree.push(node)
      }
    }
  ...
  async collectChildren(parentFolder: Folder, wholeTree: Node[]): Promise<void> {
    ...
    if (!folderWithChildren?.folderPath?.length) {
      ...
      const parentNode = getParentFolder(folderWithChildren)
      folderWithChildren.folderPath = parentNode
        ? `${await getNodePath(this.em, parentNode)}/`
        : '/'
    }
