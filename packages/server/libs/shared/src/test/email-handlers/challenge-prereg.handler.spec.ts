...
describe('ChallengePreregEmailHandler', () => {
  ...
  const getHandler = (): ChallengePreregEmailHandler => {
    return new ChallengePreregEmailHandler(entityManager, challengeRepo, userRepo, spaceMembershipRepo, emailClient)
  }
  ...
  describe('#sendEmail', () => {
    ...
    it('public', async () => {
      ...
      expect(emailClientSendEmailStub.firstCall.args[0].emailType).to.equal(EMAIL_TYPES.challengePrereg)
