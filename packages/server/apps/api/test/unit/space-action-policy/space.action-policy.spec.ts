...
describe('Space.action-policy ', () => {
  ...
  beforeEach(async () => {
    ...
    regularMembership = create.spacesHelper.addMember(
      em,
  ...
  context('canAccept()', () => {
    it('should return true - not accepted review space and user is a lead', async () => {
      const confidentailSpace1 = create.spacesHelper.create(em, {
        spaceId: reviewSpace.id,
        type: SPACE_TYPE.REVIEW,
        hostDxOrg: `org-pfda..space_host_${random.dxstr()}`,
      })
      const confidentailSpace2 = create.spacesHelper.create(em, {
        spaceId: reviewSpace.id,
        type: SPACE_TYPE.REVIEW,
