...
describe('GET /files', () => {
  ...
  it('fetch files with filters', async () => {
    create.filesHelper.create(em, { user }, { name: 'file1.txt', scope: 'private', fileSize: 100 })
    ...
    create.filesHelper.create(
      em,
      { user },
      { name: 'file4name.txt', scope: 'private', fileSize: 400, state: FILE_STATE_DX.OPEN },
    )
    await em.flush()
