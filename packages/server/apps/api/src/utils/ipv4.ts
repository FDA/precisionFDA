...
export const ipv4QuadrupleToBooleanArray: (ipv4Quadruple: number[]) => boolean[] = (ipv4Quadruple: number[]) =>
  ipv4Quadruple.flatMap(n => Array.from(n.toString(2).padStart(8)).map(c => c === '1'))

export const ipv4StringToQuadruple: (ipv4String: string | undefined) => number[] = (ipv4String: string | undefined) => {
  try {
    return parseIpv4Address(ipv4String)
  } catch (validationError) {
    throw new InvalidIpHeaderError(`Invalid IPv4 address parsed from '${config.api.nginxIpHeader}' header`, {
      validationError,
    })
  }
}
