...
export class UserFileResolverFacade {
  ...
  private mapNodeResponse(
    ...
    return nodes.map((node) => ({
      ...
      type: type,
      uid: node.uid,
      dxid: node.isFolder ? null : (node as UserFile).dxid,
      state: node.state,
      file_size: node.isFolder ? undefined : (node as UserFile).fileSize,
